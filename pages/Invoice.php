...
<main class="main container" id="main">
    ...
    <div class="table-wrapper">
                    ...
                    <div class="col-sm-9">
                        ...
                        <div class="filter-group">
                            ...
                            <select class="form-control" name="sort_price" onchange="this.form.submit()">
                                <option value="" <?= $sortPrice === '' ? 'selected' : '' ?>>Select</option>
                                <option value="highest" <?= $sortPrice === 'highest' ? 'selected' : '' ?>>Highest</option>
                                <option value="lowest" <?= $sortPrice === 'lowest' ? 'selected' : '' ?>>Lowest</option>
                            </select>
                        </div>
                        <div class="filter-group">
                            ...
                            <select class="form-control" name="category_filter" onchange="this.form.submit()">
                                ...
                                while ($category = $categories->fetch(PDO::FETCH_ASSOC)) {
                                    $selected = isset($_GET['category_filter']) && $_GET['category_filter'] == $category['category_id'] ? 'selected' : '';
                ...
                <tbody>
                    ...
                    while ($row = $stmt->fetch()) {
                        ...
                        <tr>
                            ...
                            <td><button class="Stocks-box btn-secondary align-items-center">
        ...
        <!-- Pagination -->
        <div class="clearfix">
            <div class="hint-text">Showing <b><?= $stmt->rowCount() ?></b> out of <b><?= $totalItems ?></b> Products
            </div>
            <ul class="pagination">
                <li class="page-item <?= $currentPageNumber == 1 ? 'disabled' : '' ?>">
                    <a href="?page=<?= $currentPageNumber - 1 ?>&items_per_page=<?= $itemsPerPage ?>&search=<?= htmlspecialchars($searchKeyword) ?>&sort_price=<?= htmlspecialchars($sortPrice) ?>"
                        class="page-link">Previous</a>
                </li>
                <?php for ($i = 1; $i <= $totalPages; $i++): ?>
                    <li class="page-item <?= $currentPageNumber == $i ? 'active' : '' ?>">
                        <a href="?page=<?= $i ?>&items_per_page=<?= $itemsPerPage ?>&search=<?= htmlspecialchars($searchKeyword) ?>&sort_price=<?= htmlspecialchars($sortPrice) ?>"
                            class="page-link"><?= $i ?></a>
                    </li>
                <?php endfor; ?>
...
<script>
    $(document).ready(function () {
        ...
        $('input[name="search"]').on('keyup', function () {
            const searchValue = $(this).val(); // Get the current search value
            const itemsPerPage = $('select[name="items_per_page"]').val(); // Items per page value
            const sortPrice = $('select[name="sort_price"]').val(); // Sort price value
            const categoryFilter = $('select[name="category_filter"]').val(); // Category filter value

            // Send an AJAX request to fetch filtered products
            $.ajax({
                url: "<?= basename($_SERVER['PHP_SELF']); ?>", // Current page
                type: "GET",
                data: {
                    search: searchValue,
                ...
                success: function (response) {
                    ...
                    const newTableContent = $(response).find('.table-responsive').html();
                    ...
                    const newHintText = $(response).find('.hint-text').html();
                    $('.hint-text').html(newHintText);
                },
                error: function () {
                    toastr.error("Error fetching search results. Please try again.");
                }
            });
        });
    });
